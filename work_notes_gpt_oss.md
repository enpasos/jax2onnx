...
- 2025-11-08 (routing parity harness):
  - Cloned `atveit/gpt-oss-jax-vs-torch-numerical-comparison` under `tmp/` and mirrored its expert-routing comparison script into `scripts/gpt_oss_routing_parity.py`.
  - The new harness accepts `--gpt-oss-path`, `--jax-checkpoint`, `--torch-checkpoint`, and `--prompt`, then runs both references while capturing per-token expert IDs + gate weights. Results are printed and written to `artifacts/gpt_oss_routing/`.
  - This gives us a reproducible way to validate Flax vs. PyTorch numerics (bf16 tolerances, expert match rates) before touching the ONNX path. Next iteration: wire the capture hooks into our Flax/NNX modules so the harness can operate on our in-repo implementation instead of the upstream clone.
- 2025-11-08 (weights + capture plumbing):
  - Converted the Hugging Face checkpoint at `~/.cache/gpt_oss/gpt-oss-20b/original` into an Orbax snapshot (`~/.cache/gpt_oss/gpt-oss-20b/orbax`) using `python -m gpt_oss.jax.scripts.convert_checkpoint`. This gives us fast-loading weights for the parity harness.
  - Added optional `capture_routing` plumbing to the Flax `MLPBlock`/`TransformerBlock` in `jax2onnx/plugins/examples/nnx/gpt_oss_flax.py`. When provided, the block now appends host-side copies of `expert_ids` and `gate_weights`, mirroring the hooks in PR #217. Default behaviour (used by ONNX tests) is unchanged.
  - First attempt to run `scripts/gpt_oss_routing_parity.py` against the 20B checkpoints (CPU-only JAX) timed out after ~95 minutes during JAX model execution. Need to investigate lighter-weight options (e.g., layer limiting or GPU-backed env) before we can capture full-layer parity locally.
- Added a Poetry `gpu` dependency group in `pyproject.toml`; `poetry install --with gpu` now pulls the CUDA 12 JAX wheels (`jax[cuda12_pip]`) so we can run the parity harness on machines with a 4090 without polluting the default install.
  - `scripts/gpt_oss_routing_parity.py` now accepts `--jax-platform`, `--max-layers`, and `--max-tokens`, letting us force CPU-only runs and trim comparisons to the early layers/tokens so we can iterate on parity without exhausting local hardware.
  - Full CPU sweep (`--max-layers 24 --max-tokens 4 --torch-device cpu`) shows 20/24 layers still hitting the same experts with gate diffs ≤1.17e-2 (layers 7/10/13/16 diverge only because of bf16 tie-breaking). Latest report: `artifacts/gpt_oss_routing/flax/20251110-173511_summary.md`.
- Flax weight-export plan (to reach trained ONNX baseline):
  1. **Weight loader:** port the PR #217 Flax checkpoint loader into `scripts/` (or a helpers module) so we can load safetensors/orbax directly into the `FlaxTransformer` example.
  2. **Export script:** build `scripts/export_flax_gpt_oss_example_with_mapped_weights.py` mirroring the Equinox exporter (stage weights → emit ONNX via `construct_and_call`).
  3. **Parity verification:** run the harness on the weight-mapped Flax model, then feed the ONNX graph through the existing expect_graph/policy tests.
  4. **Documentation:** record the full workflow (download → load → export → parity) in `docs/readme/gpt_oss/getting_weights.md` once the scripts land.
  - Latest CPU-only run (`--max-layers 16 --max-tokens 4 --torch-device cpu`) shows layers 0–6/8–9/11–12/14–15 still match JAX↔PyTorch exactly, with gate diffs ≤9.8e-3; divergences at layers 7/10/13 are within the bf16 decision-boundary window documented in atveit's parity repo. Summary written to `artifacts/gpt_oss_routing/flax/20251110-171737_summary.md` for reference.
