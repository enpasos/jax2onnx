...
1. **Plugin-based extensibility**: All operator-specific logic lives in plugins. The core converter knows nothing about specific operations like convolutions or attention—it only orchestrates the conversion process.
2. **Minimal core, maximal flexibility**: The converter is a thin, generic engine that traces JAX programs and delegates lowering to plugins, keeping the architecture clean and maintainable.
...
* [`advanced_topics/onnx_ir_builder.md`](advanced_topics/onnx_ir_builder.md) – canonical builder guardrails and examples.
* [`advanced_topics/expect_graph_reference.md`](advanced_topics/expect_graph_reference.md) – structural test patterns for `expect_graph`.
* [`advanced_topics/subgraph_input_handling.md`](advanced_topics/subgraph_input_handling.md) – control-flow body wiring (If/Loop/Scan).
* [`../user_guide/supported_components.md`](../user_guide/supported_components.md) – autogenerated support matrices for primitives/examples.
* [`../about/past_versions.md`](../about/past_versions.md) – changelog snapshots for each jax2onnx release.
...
* **Plugin discovery.** Recursively import [`jax2onnx/plugins/`](https://github.com/enpasos/jax2onnx/tree/main/jax2onnx/plugins) (pattern: `plugins/*`). Plugins self-register into a registry keyed by **primitive name** (string). The core never sees concrete classes like `nnx.Conv`.
...
* **IR assembly.** Walk equations in order; for each equation:
  ...
  * Converters/plugins emit new ops through `ctx.builder` so constants and `_outputs` stay consistent across ONNX IR variants (see [ONNX IR Builder Guide](advanced_topics/onnx_ir_builder.md)).
...
- **Builder-first**: emit ops via `ctx.builder` (see [ONNX IR Builder Guide](advanced_topics/onnx_ir_builder.md)). `_outputs` must always be a sequence; constants come from `builder.initializer(...)` or `ctx.bind_const_for_var(...)`.
...
- **Single-use RNG / module construction**: never seed at import time. Expose stochastic callables with `construct_and_call(...).with_rng_seed(...)` / `.with_requested_dtype(...)` so the test harness can rebuild modules for both f32/f64 variants without clashes (see [AGENTS.md](https://github.com/enpasos/jax2onnx/blob/main/AGENTS.md)).
- **No protobuf in converters/plugins**: only the top-level adapters touch `onnx` (protobuf types). Policy tests under [`tests/extra_tests`](https://github.com/enpasos/jax2onnx/tree/main/tests/extra_tests) enforce this.
...
The full conversion pipeline spans two modules: [`conversion_api.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/converter/conversion_api.py) (core conversion) and [`user_interface.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/user_interface.py) (export facade). The following table shows the exact order:
...
|--------|----------------|
| [`ir_optimizations.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/converter/ir_optimizations.py) | Pure optimization passes (DCE, CSE, constant lifting, reshape folding) |
| [`conversion_api.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/converter/conversion_api.py) | Core conversion + optimization + finalization (returns precise-shape model) |
| [`ir_postprocess.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/converter/ir_postprocess.py) | Export preparation: shape loosening for runtime flexibility |
| [`user_interface.py`](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/user_interface.py) | Public API facade: orchestrates conversion → postprocess → serialize |
...
* The builder records nodes/initializers in one place and mirrors `_tape.Builder`
  semantics across converters, plugins, and tests. The policy suite ([`tests/extra_tests/framework/test_ir_builder_contracts.py`](https://github.com/enpasos/jax2onnx/blob/main/tests/extra_tests/framework/test_ir_builder_contracts.py)
  and [`scripts/check_ir_builder_usage.py`](https://github.com/enpasos/jax2onnx/blob/main/scripts/check_ir_builder_usage.py)) assumes `_outputs`/initializer calls flow through
  the builder helpers.
...
* The [ONNX IR Builder Guide](advanced_topics/onnx_ir_builder.md) collects the concrete guardrails and
...
* When the lowered structure changes, run `poetry run python scripts/emit_expect_graph.py <testcase>` to regenerate a canonical snippet; paste the result into the plugin metadata (script: [`scripts/emit_expect_graph.py`](https://github.com/enpasos/jax2onnx/blob/main/scripts/emit_expect_graph.py)).
...
Before serialization we run a tiny, structure-only optimization sweep. The canonical rules and implementation notes live in [`advanced_topics/ir_optimizer.md`](advanced_topics/ir_optimizer.md). Today the only pass folds redundant `Transpose → [elementwise]* → Transpose` pairs when their permutations compose to identity; future passes must follow the same IR-only, backend-agnostic constraints.
...
See [`advanced_topics/expect_graph_reference.md`](advanced_topics/expect_graph_reference.md) for a focused reference on writing
