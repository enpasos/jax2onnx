...
* [Supported Components](../user_guide/supported_components.md) – autogenerated support matrices for primitives/examples.
* [ONNX Operator Coverage](../user_guide/onnx_operator_coverage.md) – ONNX-first matrix cross-checking official operators against plugin usage.
* [Past Versions](../about/past_versions.md) – changelog snapshots for each jax2onnx release.
...
* **Plugin discovery.** Recursively import the [plugins directory](https://github.com/enpasos/jax2onnx/tree/main/jax2onnx/plugins) (pattern: `plugins/*`). Plugins self-register into a registry keyed by **primitive name** (string). The core never sees concrete classes like `nnx.Conv`.
...
- **Single-use RNG / module construction**: never seed at import time. Expose stochastic callables with `construct_and_call(...).with_rng_seed(...)` / `.with_requested_dtype(...)` so the test harness can rebuild modules for both f32/f64 variants without clashes (see [agent guidelines](https://github.com/enpasos/jax2onnx/blob/main/AGENTS.md)).
- **No protobuf in converters/plugins**: only the top-level adapters touch `onnx` (protobuf types). Policy tests under [policy tests](https://github.com/enpasos/jax2onnx/tree/main/tests/extra_tests) enforce this.
...
The full conversion pipeline spans two modules: [Conversion API](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/converter/conversion_api.py) (core conversion) and [User Interface](https://github.com/enpasos/jax2onnx/blob/main/jax2onnx/user_interface.py) (export facade). The following table shows the exact order:
...
* The builder records nodes/initializers in one place and mirrors `_tape.Builder`
  semantics across converters, plugins, and tests. The policy suite ([IR builder contract test](https://github.com/enpasos/jax2onnx/blob/main/tests/extra_tests/framework/test_ir_builder_contracts.py)
  and [IR builder usage checker](https://github.com/enpasos/jax2onnx/blob/main/scripts/check_ir_builder_usage.py)) assumes `_outputs`/initializer calls flow through
  the builder helpers.
...
* When the lowered structure changes, run `poetry run python scripts/emit_expect_graph.py <testcase>` to regenerate a canonical snippet; paste the result into the plugin metadata (script: [expect_graph emitter](https://github.com/enpasos/jax2onnx/blob/main/scripts/emit_expect_graph.py)).
...
See [Expect Graph Reference](advanced_topics/expect_graph_reference.md) for a focused reference on writing
