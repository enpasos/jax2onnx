...
- `jax.lax.scan` is currently lowered through ONNX `Loop`, not the ONNX `Scan` operator. Scanned inputs are passed explicitly into the loop body and sliced by iteration; nothing is implicitly captured.
...
## `jax.lax.scan`: Loop lowering with explicit sequence state
`jax.lax.scan` lowers to ONNX `Loop`. When `xs=None`, the loop body receives the
standard Loop header plus explicit constant/carry state. When scan inputs are
present, the outer sequence tensors are passed explicitly as loop-carried state;
the body gathers the current per-step slice by iteration index and returns the
sequence state unchanged. It yields updated carry values plus scan outputs that
the parent graph stacks or reshapes as required. Tests under
`tests/extra_tests/scan/` assert these invariants so regressions surface quickly.
...
- When authoring tests, assert Loop node/body arity directly for control-flow
  body contracts; use `expect_graph` for top-level structural paths and imported
