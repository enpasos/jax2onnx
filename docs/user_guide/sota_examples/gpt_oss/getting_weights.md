...
Unless you specifically need the Equinox version, follow the Flax/NNX workflow
below. The Equinox workflow is listed separately at the end.
...
- Pass `--config /path/to/config.json` if the staging script’s JSON lives
  elsewhere.
- The exporter includes validation by default. Use `--skip-validation` only when
  you explicitly want to inspect an exported graph before runtime validation.
...
For a short-window ONNX-only run (no JAX/Torch in memory), export a small model
and run tokenizer-backed generation:
...
modules, and emit an ONNX graph. On memory constrained systems it helps to run
the export in two stages:
