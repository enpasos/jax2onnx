...
| Flax NNX activations / pooling / conv | ⏳ mixed | `relu`/`gelu`/`elu`/`tanh`/`softplus`/`softmax`/`sigmoid`/`avg_pool`/`max_pool` migrated; conv + batch/layer/group/RMS norm now builder-only. |
