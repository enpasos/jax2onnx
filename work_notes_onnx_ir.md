...
Progress: lax `Add`, `Mul`, `Sub`, `Div`, `Max`, `Min`, `Sin`, `Cos`, `Log`, `Tanh`, `Exp`, `Abs`, `Sqrt`, `Sign`, and `Logistic` now emit nodes through `ctx.builder`, providing reference patterns for remaining elementwise lowers.
