...
def get_primitive():
    """Returns the jnp.reshape primitive."""
    return jnp.reshape_p


def process_newshape(newshape: Sequence[Union[int, str]]) -> List[Union[int, str]]:
...
def get_dynamic_output_shape(
    ...
    newshape = process_newshape(newshape)
...
def reshape_abstract_eval(a, newshape):
    ...
    newshape_processed = process_newshape(newshape)
    output_shape = get_dynamic_output_shape(a.shape, newshape_processed)
...
def patch_info():
    ...
    return {
        "patch_targets": [jnp],
        "patch_function": lambda _: reshape,
...
def get_handler(s: "Jaxpr2OnnxConverter"):
    def handle_reshape(node_inputs, node_outputs, params):
        ...
        output_shape = get_dynamic_output_shape(input_shape, newshape)
        processed_newshape = process_newshape(newshape)
