...
class LowerDimExpr:
    ...
    def _get_dim_value(self, name: str) -> ir.Value:
        ...
        shp.type = ir.TensorType(ir.DataType.INT64) # this is defined by ONNX specs to be INT64 for Shape
    ...
    def _convert_op(self, name: str, operands: list[ir.Value]) -> ir.Value:
        if name == "floordiv":
            result = self.ctx.builder.Div(operands[0], operands[1], _outputs=[self.ctx.fresh_name("dimexpr_div")])
        elif name == "max":
            result = self.ctx.builder.Max(*operands, _outputs=[self.ctx.fresh_name("dimexpr_max")])
        elif name == "min":
            result = self.ctx.builder.Min(*operands, _outputs=[self.ctx.fresh_name("dimexpr_min")])
        elif name == "mod":
            result = self.ctx.builder.Mod(*operands, _outputs=[self.ctx.fresh_name("dimexpr_mod")])
        else:
