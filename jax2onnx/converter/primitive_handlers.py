"""
JAX Primitive Handlers Module

This module provides handlers for converting JAX primitives to their ONNX equivalents.
It contains the PrimitiveDispatcher class which manages the dispatching of JAX primitives
to their corresponding handler functions during the ONNX conversion process.
"""
...
class PrimitiveDispatcher:
    ...
    This class provides a registry of handlers for built-in JAX primitives and
    dispatches JAX primitive operations to the appropriate handler function.
    It serves as a core component in the conversion of JAX operations to ONNX.
    """
    ...
    def _define_builtin_handlers(self):
        """Register handlers for built-in JAX primitives."""
    ...
    def dispatch_and_execute(self, converter: "Jaxpr2OnnxConverter", eqn: JaxprEqn):
        """
        Dispatches a JAX primitive to its corresponding handler and executes it.

        Args:
            converter: The Jaxpr2OnnxConverter instance
            eqn: The JAX equation (JaxprEqn) to process

        Raises:
            NotImplementedError: If no handler is registered for the primitive
        """
    ...
    def _create_identity_node(self, converter: "Jaxpr2OnnxConverter", eqn: JaxprEqn):
        """
        Creates an Identity node in the ONNX graph.

        Args:
            converter: The Jaxpr2OnnxConverter instance
            eqn: The JAX equation to create an Identity node for
        """
    ...
    def _handle_random_seed(self, converter, eqn, **params):
        """Handler for the random_seed primitive."""
        self._create_identity_node(converter, eqn)

    def _handle_random_wrap(self, converter, eqn, **params):
        """Handler for the random_wrap primitive."""
        self._create_identity_node(converter, eqn)

    def _handle_random_unwrap(self, converter, eqn, **params):
        """Handler for the random_unwrap primitive."""
        self._create_identity_node(converter, eqn)

    def _handle_random_split(self, converter, eqn, **params):
        """
        Handler for the random_split primitive.

        This handler implements random key splitting using Reshape and Tile operations.

        Args:
            converter: The Jaxpr2OnnxConverter instance
            eqn: The JAX equation to process
    ...
    def _handle_convert_element_type(self, converter, eqn, **params):
        """
        Handler for the convert_element_type primitive.

        This handler creates a Cast node in the ONNX graph to change data types.

        Args:
            converter: The Jaxpr2OnnxConverter instance
            eqn: The JAX equation to process
            **params: Parameters for the primitive, including 'new_dtype'
        """
        input_names = [converter.get_name(inp) for inp in eqn.invars]
        output_name = converter.get_name(eqn.outvars[0])
    ...
    def _handle_device_put(self, converter, eqn, **params):
        """
        Handler for the device_put primitive.

        This handler handles moving values to a specific device in JAX.
        For constants, it creates an actual constant node in ONNX.
        For variables, it creates an Identity node.
