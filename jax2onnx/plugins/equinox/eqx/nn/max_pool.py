# jax2onnx/plugins/equinox/eqx/nn/max_pool.py
...
_EQX_MAX_POOL_2D: Final[eqx.nn.MaxPool2d] = eqx.nn.MaxPool2d(
...
def _max_pool_forward(
    ...
    *,
    ...
    if not jnp.issubdtype(arr.dtype, jnp.floating):
        raise TypeError("Eqx MaxPool plugin currently supports floating dtypes only.")
    ...
    if arr.ndim == num_spatial + 1:
        ...
        raise ValueError(
            f"eqx MaxPool expects rank {num_spatial + 1} (no batch) or "
...
@register_primitive(
    jaxpr_primitive="eqx.nn.max_pool",
    jax_doc="https://docs.kidger.site/equinox/api/nn/pooling/#equinox.nn.MaxPool2d",
    onnx=[
        {
            "component": "MaxPool",
    ...
    testcases=[
        {
            "testcase": "eqx_max_pool2d_basic",
            "callable": _EQX_MAX_POOL_2D,
            "input_shapes": [(3, 8, 8)],
        ...
        {
            "testcase": "eqx_max_pool2d_batched",
            "callable": lambda x, _mod=_EQX_MAX_POOL_2D: jax.vmap(_mod)(x),
...
class MaxPoolPlugin(PrimitiveLeafPlugin):
    _PRIM: ClassVar[Primitive] = Primitive("eqx.nn.max_pool")
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: jax_core.JaxprEqn) -> None:
        ...
        if builder is None:
            raise AttributeError(
                "IR build context missing builder for Eqx MaxPool lowering"
            )
        ...
        x_val = ctx.get_value_for_var(x_var, name_hint=ctx.fresh_name("maxpool_x"))
        out_spec = ctx.get_value_for_var(
            out_var, name_hint=ctx.fresh_name("maxpool_out")
        )
        ...
        if rank not in {num_spatial + 1, num_spatial + 2}:
            raise ValueError(
                f"Eqx MaxPool lowering expects rank {num_spatial + 1} or "
        ...
        if rank == num_spatial + 1:
            unsq_axes = _const_i64(ctx, [0], name_hint="eqx_maxpool_unsq_axes")
            pooled_input = builder.Unsqueeze(
                x_val,
                unsq_axes,
                _outputs=[ctx.fresh_name("eqx_maxpool_unsqueeze")],
            )
        ...
        pooled_name = (
            ctx.fresh_name("eqx_maxpool_pooled")
            if rank == num_spatial + 1
            else (getattr(out_spec, "name", None) or ctx.fresh_name("eqx_maxpool"))
        )
        ...
        if rank == num_spatial + 1:
            squeeze_axes = _const_i64(ctx, [0], name_hint="eqx_maxpool_sq_axes")
    ...
    def binding_specs(cls) -> list[AssignSpec | MonkeyPatchSpec]:
        return [
            AssignSpec("equinox.nn", "max_pool_p", cls._PRIM, delete_if_missing=True),
            MonkeyPatchSpec(
                target="equinox.nn.MaxPool1d",
                attr="__call__",
                make_value=lambda orig: cls._patch_call(orig),
                delete_if_missing=False,
            ),
            MonkeyPatchSpec(
                target="equinox.nn.MaxPool2d",
                attr="__call__",
                make_value=lambda orig: cls._patch_call(orig),
                delete_if_missing=False,
            ),
            MonkeyPatchSpec(
                target="equinox.nn.MaxPool3d",
                attr="__call__",
                make_value=lambda orig: cls._patch_call(orig),
                delete_if_missing=False,
            ),
    ...
    def _patch_call(
        ...
        def wrapped(
            ...
            return MaxPoolPlugin._PRIM.bind(
                x,
                kernel_size=kernel_size,
                strides=strides,
                padding=padding,
...
batching.primitive_batchers[MaxPoolPlugin._PRIM] = _max_pool_batch_rule
