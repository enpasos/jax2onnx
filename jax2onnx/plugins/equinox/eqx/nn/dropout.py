...
# Back-compat helper: JAX >= 0.4 exposes random.key; older uses PRNGKey
def _make_key(seed: int = 0):
    try:
        return jax.random.key(seed)  # JAX newer API
    except AttributeError:
        return jax.random.PRNGKey(seed)  # legacy API
