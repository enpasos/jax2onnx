...
class LinearPlugin(PrimitiveLeafPlugin):
    ...
    def abstract_eval(
        ...
        def _helper(xv, kv, bv):
            """Helper function that executes the actual dot_general + bias."""
            return lax.dot_general(xv, kv, dimension_numbers) + bv
    ...
    def to_onnx(
        ...
        # Important: keep the variable names as they are for validation
        x_name = s.get_name(x_var)
        k_name = s.get_name(k_var)
        b_name = s.get_name(b_var)
        y_name = s.get_name(y_var)

        # Register the input tensor information
        self._ensure_graph_input(s, x_name, x_var)
        s.add_shape_info(x_name, tuple(x_var.aval.shape), x_var.aval.dtype)

        # --------------------- flatten leading batch dims if needed ---------------
        ...
        # -------------------- kernel and bias handling ---------------------------
        # Extract kernel (weight) matrix and convert to constant
        kernel_const = None
        if hasattr(k_var, "val"):
            kernel_const = np.asarray(k_var.val)
        elif k_name in s.name_to_const:
            kernel_const = s.name_to_const[k_name]
        else:
            logger.warning(f"No kernel value found for {k_name}, using zero weights")
            kernel_const = np.zeros(k_var.aval.shape, dtype=np.float32)

        weight_name = s.get_constant_name(kernel_const)

        # Extract bias vector and convert to constant
        bias_const = None
        if hasattr(b_var, "val"):
            bias_const = np.asarray(b_var.val)
        elif b_name in s.name_to_const:
            bias_const = s.name_to_const[b_name]
        else:
            logger.warning(f"No bias value found for {b_name}, using zero bias")
            bias_const = np.zeros(b_var.aval.shape, dtype=np.float32)
        ...
        gemm_node = helper.make_node(
            "Gemm",
            inputs=[x2d_name, weight_name, bias_name],
            outputs=[gemm_out],
        ...
        if len(batch_dims) > 1:
            ...
            # For 2D case, the output is directly from GEMM
