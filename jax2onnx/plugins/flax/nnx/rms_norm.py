...
class RMSNormPlugin(PrimitiveLeafPlugin):
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: jax.core.JaxprEqn) -> None:
        ...
        opset = int(builder.opset)

        dims: tuple[str | int | None, ...] = tuple(
        ...
        if axis < len(mean_shape):
            mean_shape[axis] = 1
        mean_dims = tuple(_to_ir_dim_for_shape(d) for d in mean_shape)
    ...
    @staticmethod
    def _prepare_scale(scale_obj: Any | None, size: int, dtype: Any) -> jax.Array:
    ...
    def _patch_call(cls, orig: Callable[..., Any]) -> Callable[..., Any]:
        def wrapped(self: nnx.RMSNorm, x: Any, mask: Any | None = None) -> Any:
            if mask is not None:
