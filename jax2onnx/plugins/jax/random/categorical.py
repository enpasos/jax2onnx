...
class RandomCategoricalPlugin(PrimitiveLeafPlugin):
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: jax.core.JaxprEqn) -> None:
        ...
        batch_shape = tuple(int(d) for d in logits_shape[:-1])
        num_distributions = int(np.prod(batch_shape)) if batch_shape else 1
        ...
        reshape_2d_shape = ctx.builder.add_initializer_from_array(
            ...
            array=np.asarray([num_distributions, int(class_count)], dtype=np.int64),
        )
        ...
        out_shape_const = ctx.builder.add_initializer_from_array(
