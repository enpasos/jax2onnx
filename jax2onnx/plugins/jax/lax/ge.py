...
class GePlugin(PrimitiveLeafPlugin):
    def lower(self, ctx: LoweringContextProtocol, eqn: "core.JaxprEqn") -> None:
        ...
        out_spec = ctx.get_value_for_var(out_var, name_hint=ctx.fresh_name("ge_out"))

        desired_name = getattr(out_spec, "name", None) or ctx.fresh_name("ge_out")
        producer = getattr(out_spec, "producer", lambda: None)
        if callable(producer) and producer() is not None:
            desired_name = ctx.fresh_name("ge_out")

        result = ctx.builder.GreaterOrEqual(lhs_val, rhs_val, _outputs=[desired_name])
