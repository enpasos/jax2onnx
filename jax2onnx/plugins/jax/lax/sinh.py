...
@register_primitive(
    ...
    onnx=[{"component": "Sinh", "doc": "https://onnx.ai/onnx/operators/onnx__Sinh.html"}],
...
class SinhPlugin(PrimitiveLeafPlugin):
    def to_onnx(self, s: "Jaxpr2OnnxConverter", node_inputs, node_outputs, params):
        ...
        if dtype == np.float32:
            ...
            s.add_node(helper.make_node("Exp", inputs=[neg_x_name], outputs=[exp_neg_x_name]))
            s.add_shape_info(exp_neg_x_name, x.aval.shape, dtype)
            
            # exp(x) - exp(-x)
            diff_name = s.get_unique_name("diff")
            s.add_node(helper.make_node("Sub", inputs=[exp_x_name, exp_neg_x_name], outputs=[diff_name]))
            ...
            # (exp(x) - exp(-x)) * 0.5
            node = helper.make_node("Mul", inputs=[diff_name, half_const_name], outputs=[out_name])
            s.add_node(node)