...
class ReduceWindowSumPlugin(PrimitiveLeafPlugin):
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: Any) -> None:
        ...
        if np_dtype not in self._FLOAT_LIKE_DTYPES:
            ...
            if conv_dtype_enum is None:
                raise TypeError(
                    f"Unsupported convolution dtype for reduce_window_sum: {conv_np_dtype}"
                )
        ...
        squeezed = cast(
            ...
            ctx.builder.Squeeze(
                ...
                squeeze_axes,
                _outputs=[
                    getattr(out_val, "name", None)
                    or ctx.fresh_name("reduce_window_sum")
                ],
            ),
        )
        ...
        if needs_cast_back:
            ...
            if out_dtype_enum is None:
                raise TypeError(
                    f"Unsupported output dtype for reduce_window_sum: {np_dtype}"
                )
            cast_out = cast(
                ir.Value,
                ctx.builder.Cast(
    ...
    def _apply_base_dilation(
        ...
        tensor: ir.Value,
        operand_shape: Sequence[DimInput],
        base_dilation: tuple[int, ...],
        conv_np_dtype: np.dtype[Any],
        conv_dtype_enum: ir.DataType,
    ) -> tuple[ir.Value, tuple[DimInput, ...]]:
        ...
        static_operand_shape = tuple(cast(int, dim) for dim in operand_shape)
        shape_list = [int(dim) for dim in static_operand_shape]
        original_dims = [int(dim) for dim in static_operand_shape]
        builder = ctx.builder
        work_val = tensor
        ...
        return work_val, tuple(int(d) for d in shape_list)
