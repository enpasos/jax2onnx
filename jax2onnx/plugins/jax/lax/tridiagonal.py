...
def _scatter_vec_elem(
    ...
    out = cast(
        ir.Value,
        ctx.builder.ScatterND(vec, idx, value_1d, _outputs=[ctx.fresh_name(name)]),
    )
    _stamp_like(out, vec)
    return out
...
class TridiagonalPlugin(PrimitiveLeafPlugin):
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: Any) -> None:
        ...
        a_out = cast(ir.Value, ctx.builder.Identity(x, _outputs=[a_name]))
        ...
        if getattr(d_out, "name", None) != d_name:
            d_out = cast(ir.Value, ctx.builder.Identity(d_vec, _outputs=[d_name]))
        ...
        if getattr(e_out, "name", None) != e_name:
            e_out = cast(ir.Value, ctx.builder.Identity(e_vec, _outputs=[e_name]))
        ...
        if getattr(taus_out, "name", None) != taus_name:
            taus_out = cast(
                ir.Value, ctx.builder.Identity(taus_vec, _outputs=[taus_name])
            )
        _stamp_like(
