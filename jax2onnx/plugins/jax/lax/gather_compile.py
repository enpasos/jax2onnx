...
def lax_gather_to_gir(eqn: Any, indices_var_value: Optional[np.ndarray]) -> list[dict]:
    ...
    all_output_positions = set(range(len(output_shape)))

    # Extract indices shape (drop trailing index axis)
    indices_shape_with_batchdims = list(indices_var.aval.shape[:-1])
    indices_shape = [dimshape for dimidx,dimshape in enumerate(indices_shape_with_batchdims) if dimidx not in set(dn.start_indices_batching_dims)]
        ...
        # Passthrough or range_slice
            ...
            if slice_sizes[dim] == operand_shape[dim]: # whole slice -> passthrough
                ...
                if dim in dn.start_index_map:
                    indices_var_idx = dn.start_index_map.index(dim)
                    if indices_var_value is not None and check_uniform_start_index(indices_var_value, indices_var_idx):
                        entry["start_offset_value"] = int(np.reshape(indices_var_value[..., indices_var_idx], (-1,))[0]) # so we know the static uniform start index, this means this is a simple slice operation
                    else:
                        entry["mode"] = "dynamic_range_slice"
                    entry["start_indices_var_index"] = indices_var_idx # may be needed to remove the dimension from the constant indices
                entry["size"] = slice_sizes[dim]
            entry["target_dimensions_shape"] = [slice_sizes[dim]]
        
        gir_entries.append(entry)
    
    slice_dims = [dim for dim in gir_entries if dim["mode"] in ["passthrough", "range_slice", "dynamic_range_slice"]]
    ...
    for operand_dim_index, indices_dim_index in zip(dn.operand_batching_dims,dn.start_indices_batching_dims):
        indices_dims_from_input_dims[indices_dim_index] = [gir_entries[operand_dim_index]]
    ...
    if indices_var_value is not None:
        result_ops.append({"op" : "index_tensor", "value" : indices_var_value, "shape" : list(indices_var_value.shape)})
...
def extract_slicing(gir_instr_orig: dict) -> list[dict]:
    ...
    for dim in gir_instr["dims"]:
        entry = {"dim" : dim["dim"], "input_size" : dim["input_size"], "target_dimensions" : [dim["dim"]]}
    ...
    indices_after_deletion = [idx for idx in orig_index_indices if idx not in index_dimensions_to_delete]
    index_index_mapping = {orig_idx : i for i, orig_idx in enumerate(indices_after_deletion)}
    
    for dim in gir_instr["dims"]:
        if "indices_var_index" in dim:
            dim["indices_var_index"] = index_index_mapping[dim["indices_var_index"]]
        if "start_indices_var_index" in dim:
            dim["start_indices_var_index"] = index_index_mapping[dim["start_indices_var_index"]]
    
    if any(dim["mode"] == "range_slice" for dim in gir_instr_orig["dims"]):
        result_ops = []
        if indices_after_deletion != orig_index_indices:
            result_ops += index_lastdim_gather_to_gir(indices_after_deletion, index_input_shape)
...
def turn_dynamic_range_slice_to_gather(gir_instr_orig: dict) -> list[dict]:
    ...
    for dim in gir_instr["dims"]:
        if dim["mode"] == "gather":
            ...
            assert gather_target_dimensions_shape is None or gather_target_dimensions_shape == dim["target_dimensions_shape"]
            assert gather_target_dimensions is None or gather_target_dimensions == dim["target_dimensions"]
    ...
    if len(dims_to_extend) > 0:
        result += index_expand_range_gir(dims_to_extend, dim_slice_sizes, index_input_shape)
...
def normalize_gather_with_transpose(gir_instr_orig: dict) -> list[dict]:
    ...
    new_entries += [dim for dim in gir_instr["dims"] if dim["mode"] not in ["gather", "batched"]]
...
def reorder_gathered_indices(gir_instr_orig: dict) -> list[dict]:
    ...
    indices_var_indices = [dim["indices_var_index"] for dim in gir_instr["dims"] if dim["mode"] == "gather"]
...
def detect_onnx_gather(gir_instr_orig: dict) -> list[dict]:
    ...
    if target_dimensions == list(range(len(target_dimensions))):
        ...
        result = index_reshape(gather_dims[0]["target_dimensions_shape"] + [1], gather_dims[0]["target_dimensions_shape"]) + result
...
def detect_onnx_gather_nd(gir_instr_orig: dict) -> list[dict]:
    ...
    for dim in gir_instr["dims"]:
        if dim["mode"] == "batched" and prev_mode not in [None, "batched"]:
            ...
            return [gir_instr]
        elif dim["mode"] == "passthrough" and prev_mode not in ["gather", "passthrough"]:
            return [gir_instr]
...
def fold_constant_index_tensor(gir: list[dict]) -> list[dict]:
    ...
    for instr in gir:
        if instr["op"] == "index_tensor":
            ...
            changed = True
        elif instr["op"] in ["general_gather", "dynamic_range_slice", "ONNX_Gather", "ONNX_GatherND"] and index_tensor is not None and changed:
            result_ops.append({"op" : "index_tensor", "value" : copy.deepcopy(index_tensor), "shape" : list(index_tensor.shape)})
...
def run_all_passes(gir: list[dict]) -> list[dict]:
    ...
    gir = run_one_pass(gir, normalize_gather_index_tensor_with_transpose, ["general_gather"])
