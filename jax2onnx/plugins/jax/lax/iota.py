...
@register_primitive(
    ...
    onnx=[{"component": "Range", "doc": "https://onnx.ai/onnx/operators/onnx__Range.html"}],
    since="v0.4.4",
    context="primitives.lax",
    component="iota",
    testcases=[
        {"testcase": "iota_int32", "callable": lambda: lax.iota(np.int32, 5), "input_shapes": []},
        {"testcase": "iota_float32", "callable": lambda: lax.iota(np.float32, 10), "input_shapes": []},
        {"testcase": "broadcasted_iota", "callable": lambda: lax.broadcasted_iota(np.int32, (3, 4), 1), "input_shapes": []},
    ],
)
class IotaPlugin(PrimitiveLeafPlugin):
    ...
    def to_onnx(self, s: "Jaxpr2OnnxConverter", node_inputs: Sequence[Any], 
                node_outputs: Sequence[Any], params: dict[str, Any]):
        ...
        unsqueeze_axes_name = s.get_constant_name(np.array(unsqueeze_axes, dtype=np.int64))
        ...
        if all(isinstance(dim, int) for dim in shape):
            ...
            s.add_shape_info(unsqueezed_output, tuple(intermediate_shape), np.dtype(np.int64))
            ...
            raise NotImplementedError("Dynamic shape handling for iota not fully implemented")
