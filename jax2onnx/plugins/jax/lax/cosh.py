...
class CoshPlugin(PrimitiveLeafPlugin):
    ...
    def to_onnx(self, s: "Jaxpr2OnnxConverter", node_inputs, node_outputs, params):
        ...
        if dtype == np.float32:
            ...
            s.add_node(helper.make_node("Add", inputs=[exp_x_name, exp_neg_x_name], outputs=[sum_name]))
            ...
            # (exp(x) + exp(-x)) * 0.5
            node = helper.make_node("Mul", inputs=[sum_name, half_const_name], outputs=[out_name])
            s.add_node(node)