...
def check_uniform_start_index(indices_var_value: np.ndarray, indices_var_idx: int) -> bool:
...
def transpose_to_gir(indices: list[int], input_shape: list[Any]) -> list[dict]:
    ...
    return [{"op" : "transpose", "dims" : dims, "numpy_transpose" : invert_transpose(indices)}]
...
def index_transpose_to_gir(indices: list[int], input_shape: list[Any]) -> list[dict]:
    ...
    return [{"op" : "index_transpose", "dims" : dims, "numpy_transpose" : invert_transpose(indices)}]

def index_lastdim_gather_to_gir(gather_indices: list[int], input_shape: list[int]) -> list[dict]:
    return [{"op" : "index_lastdim_gather", "gather_indices" : gather_indices, "input_shape" : input_shape}]

def index_expand_range_gir(dims_to_extend: list[int], dim_slice_sizes: list[int], input_shape: list[int]) -> list[dict]:
    dims = [{"mode" : "expand", "indices_var_index" : dim_idx, "slice_size" : slice_size} for dim_idx, slice_size in zip(dims_to_extend, dim_slice_sizes)]
    return [{"op" : "index_expand", "new_dims" : dims, "input_shape" : input_shape}]

def index_reshape(input_shape: list[int], new_shape: list[int]):
    return [{"op" : "index_reshape", "input_shape" : input_shape, "output_shape" : new_shape}]
...
def get_gir_output_shape(gir_instr: dict) -> list[Any]:
    ...
    if "dims" not in gir_instr:
        ...
        for dim in gir_instr["dims"]:
            for idx, size in zip(dim["target_dimensions"], dim["target_dimensions_shape"]):
...
def is_gather_nop(gir_instr: dict) -> bool:
    ...
    return all(dim["mode"] == "passthrough" and [dim["dim"]] == dim["target_dimensions"] for dim in gir_instr["dims"])
    
def run_one_pass(gir: list[dict], one_pass: Callable[[dict], list[dict]], targets: list[str]):
    result_gir = []
    for instr in gir:
