...
class BesselI0ePlugin(PrimitiveLeafPlugin):
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: Any) -> None:
        ...
        ax = cast(
            ir.Value, ctx.builder.Abs(x, _outputs=[ctx.fresh_name("bessel_i0e_abs")])
        )
        _stamp_like(ax, x)
        ...
        neg_ax = cast(
            ir.Value,
            ctx.builder.Neg(ax, _outputs=[ctx.fresh_name("bessel_i0e_neg_ax")]),
        )
        ...
        sqrt_ax = cast(
            ir.Value,
            ctx.builder.Sqrt(ax, _outputs=[ctx.fresh_name("bessel_i0e_sqrt_ax")]),
        )
