    ...
    testcases=[
        {
            ...
            "post_check_onnx_graph": lambda m: any(n.op_type == "CumSum" for n in m.graph.node),
        },
        {
            ...
            "post_check_onnx_graph": lambda m: any(
                n.op_type == "CumSum" and any(
                    a.name == "reverse" and a.i == 1 for a in n.attribute
...
class LaxCumsumPlugin(PrimitiveLeafPlugin):
    ...
    def to_onnx(
        ...
        axis_const_name = builder.get_constant_name(np.asarray(axis_param, dtype=np.int64))
        ...
        builder.register_value_info_metadata(
            out_name,
            tuple(int(d) if isinstance(d, (int, np.integer)) else d for d in aval_out.shape),
