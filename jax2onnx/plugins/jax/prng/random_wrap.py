"""
Plugin for handling the JAX random_wrap primitive.

This plugin converts JAX's random_wrap primitive to ONNX operations.
"""
...
@register_primitive(
    jaxpr_primitive=jax._src.prng.random_wrap_p.name,
...
class RandomWrapPlugin(PrimitiveLeafPlugin):
    """Plugin for converting jax.random.wrap to ONNX Identity."""

    def to_onnx(
        self, converter: "Jaxpr2OnnxConverter", node_inputs, node_outputs, params
    ):
        """
        Convert jax.random.wrap to ONNX operations.
        ...
        node = converter.builder.create_node(
            ...
            name=converter.get_unique_name(
                f"identity_{jax._src.prng.random_wrap_p.name}"
            ),
        )
