# jax2onnx/plugins/jax/numpy/greater_equal.py
...
_GREATER_EQUAL_PRIM: Final = make_jnp_primitive("jax.numpy.greater_equal")


@register_primitive(
    jaxpr_primitive=_GREATER_EQUAL_PRIM.name,
    jax_doc="https://docs.jax.dev/en/latest/_autosummary/jax.numpy.greater_equal.html",
    onnx=[
        {
    ...
    testcases=[
        {
            "testcase": "jnp_greater_equal_basic",
            "callable": lambda x, y: jnp.greater_equal(x, y),
            "input_shapes": [(3,), (3,)],
        ...
        {
            "testcase": "jnp_greater_equal_broadcast",
            "callable": lambda x, y: jnp.greater_equal(x, y),
        ...
        {
            "testcase": "jnp_greater_equal_mixed_dtype",
            "callable": lambda x, y: jnp.greater_equal(x, y),
            "input_values": [
            ...
            "post_check_onnx_graph": EG(
                ["Cast:3 -> GreaterOrEqual:3"],
                no_unused_inputs=True,
            ),
        },
        {
            "testcase": "greater_equal_vmap_batching",
            "callable": lambda x, y: jax.vmap(jnp.greater_equal)(x, y),
            "input_shapes": [(3, 4), (3, 4)],
        },
    ],
)
class JnpGreaterEqualPlugin(PrimitiveLeafPlugin):
    _PRIM: ClassVar = _GREATER_EQUAL_PRIM
    _FUNC_NAME: ClassVar[str] = "greater_equal"
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: core.JaxprEqn) -> None:
        ...
        lhs_val = ctx.get_value_for_var(
            lhs_var,
            name_hint=ctx.fresh_name("jnp_greater_equal_lhs"),
        )
        rhs_val = ctx.get_value_for_var(
            rhs_var,
            name_hint=ctx.fresh_name("jnp_greater_equal_rhs"),
        )
        out_spec = ctx.get_value_for_var(
            out_var,
            name_hint=ctx.fresh_name("jnp_greater_equal_out"),
        )
        ...
        if lhs_dtype != target_dtype:
            lhs_cmp = ctx.builder.Cast(
                lhs_val,
                _outputs=[ctx.fresh_name("jnp_greater_equal_lhs_cast")],
        ...
        if rhs_dtype != target_dtype:
            rhs_cmp = ctx.builder.Cast(
                rhs_val,
                _outputs=[ctx.fresh_name("jnp_greater_equal_rhs_cast")],
                to=int(target_ir.value),
            )
        ...
        desired_name = getattr(out_spec, "name", None) or ctx.fresh_name(
            "jnp_greater_equal_out"
        )
        producer = getattr(out_spec, "producer", None)
        if callable(producer) and producer() is not None:
            desired_name = ctx.fresh_name("jnp_greater_equal_out")

        result = ctx.builder.GreaterOrEqual(lhs_cmp, rhs_cmp, _outputs=[desired_name])
...
@JnpGreaterEqualPlugin._PRIM.def_impl
def _greater_equal_impl(x: object, y: object) -> object:
    orig = get_orig_impl(JnpGreaterEqualPlugin._PRIM, JnpGreaterEqualPlugin._FUNC_NAME)
    return orig(x, y)


def _greater_equal_batch_rule(
    args: tuple[Any, ...], dims: tuple[Any, ...], **params: Any
) -> tuple[Any, Any]:
    return cast(
        tuple[Any, Any],
        broadcast_batcher_compat(JnpGreaterEqualPlugin._PRIM, args, dims, **params),
    )


batching.primitive_batchers[JnpGreaterEqualPlugin._PRIM] = _greater_equal_batch_rule
