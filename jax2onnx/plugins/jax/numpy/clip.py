        ...
        # Broadcasted bounds (result has shape of x)
        {
            ...
            "input_values": [
                np.array([[ -2.0, -0.5, 3.0],
                          [  1.0,  2.0, 5.0]], dtype=np.float64),  # x  (2,3)
                np.array([[ -1.0,  0.0,  0.0]], dtype=np.float64),  # lo (1,3)
                np.array([[  1.5],], dtype=np.float64),              # hi (1,1)->(2,1)
            ],
...
class ClipPlugin(PrimitiveLeafPlugin):
    ...
    def to_onnx(
        ...
        min_name = _cast_to(s, min_name, mn_dt, x_dt, ctx="ClipMin", shape_hint=v_min.aval.shape)
        max_name = _cast_to(s, max_name, mx_dt, x_dt, ctx="ClipMax", shape_hint=v_max.aval.shape)

        # y = max(x, min)
        y_name = s.builder.get_unique_name("clip_lowered_max")
        s.add_node(
            helper.make_node("Max", inputs=[x_name, min_name], outputs=[y_name],
        ...
        # z = min(y, max)
        s.add_node(
            helper.make_node("Min", inputs=[y_name, max_name], outputs=[out_name],
