# jax2onnx/plugins/jax/numpy/zeros.py
...
_ZEROS_PRIM: Final = make_jnp_primitive("jax.numpy.zeros")
...
@register_primitive(
    jaxpr_primitive=_ZEROS_PRIM.name,
    jax_doc="https://docs.jax.dev/en/latest/_autosummary/jax.numpy.zeros.html",
    onnx=[
        {
    ...
    testcases=[
        {
            "testcase": "jnp_zeros_2x3",
            "callable": lambda: jnp.zeros((2, 3), dtype=jnp.float32),
            "input_values": [],
            "post_check_onnx_graph": EG(["ConstantOfShape:2x3"], no_unused_inputs=True),
        },
        {
            "testcase": "jnp_zeros_int32",
            "callable": lambda: jnp.zeros((4,), dtype=jnp.int32),
            "input_values": [],
            "post_check_onnx_graph": EG(["ConstantOfShape:4"], no_unused_inputs=True),
        },
    ],
)
class JnpZerosPlugin(PrimitiveLeafPlugin):
    _PRIM: ClassVar = _ZEROS_PRIM
    _FUNC_NAME: ClassVar[str] = "zeros"
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: core.JaxprEqn) -> None:
        ...
        shape_tensor = _const_i64(ctx, np.asarray(shape, dtype=np.int64), "zeros_shape")
    ...
    def binding_specs(cls) -> list[AssignSpec | MonkeyPatchSpec]:
        ...
        def _make_value(
            ...
            if orig is None:
                raise RuntimeError("Original jnp.zeros not found for monkey patching")
...
@JnpZerosPlugin._PRIM.def_impl
def _zeros_impl(
    *,
    shape: tuple[int, ...],
    dtype: np.dtype[Any] | type,
) -> jax.Array:
    orig = get_orig_impl(JnpZerosPlugin._PRIM, JnpZerosPlugin._FUNC_NAME)
    return orig(shape, dtype=dtype)


JnpZerosPlugin._PRIM.def_abstract_eval(JnpZerosPlugin.abstract_eval)
