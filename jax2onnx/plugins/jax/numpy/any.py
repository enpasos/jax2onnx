# jax2onnx/plugins/jax/numpy/any.py
...
_ANY_PRIM: Final = make_jnp_primitive("jax.numpy.any")


@register_primitive(
    jaxpr_primitive=_ANY_PRIM.name,
    ...
    testcases=[
        ...
        {
            "testcase": "jnp_any_axis_keepdims",
        ...
        {
            "testcase": "any_vmap_batching",
            "callable": lambda x: jax.vmap(jnp.any)(x),
            "input_values": [np.array([[False, False], [True, False]], dtype=np.bool_)],
        },
    ],
)
class JnpAnyPlugin(PrimitiveLeafPlugin):
    _PRIM: ClassVar = _ANY_PRIM
    _FUNC_NAME: ClassVar[str] = "any"
    ...
    def binding_specs(cls) -> list[AssignSpec | MonkeyPatchSpec]:
        ...
        def _make_value(
            ...
            if orig is None:
                raise RuntimeError("Original jnp.any not found for monkey patching")
                ...
                *,
                ...
                if out is not None:
                    raise NotImplementedError(
                        "jnp.any with 'out' is not supported for ONNX export"
                    )
                if where is not None:
                    raise NotImplementedError(
                        "jnp.any with 'where' is not supported for ONNX export"
                    )
...
def _any_impl(
    ...
    *,
    ...
    orig = get_orig_impl(JnpAnyPlugin._PRIM, JnpAnyPlugin._FUNC_NAME)
    axis = axis_arg_from_params(axes, axes_is_tuple)
    return orig(jnp.asarray(a), axis=axis, keepdims=keepdims)


register_reduction_batch_rule(JnpAnyPlugin._PRIM, jax.lax.reduce_or_p)
