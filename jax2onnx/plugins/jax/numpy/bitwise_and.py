# jax2onnx/plugins/jax/numpy/bitwise_and.py
...
_BITWISE_AND_PRIM: Final = make_jnp_primitive("jax.numpy.bitwise_and")
...
@register_primitive(
    jaxpr_primitive=_BITWISE_AND_PRIM.name,
    jax_doc="https://docs.jax.dev/en/latest/_autosummary/jax.numpy.bitwise_and.html",
    onnx=[
        {
            "component": "And",
    ...
    testcases=[
        {
            "testcase": "jnp_bitwise_and_int",
            "callable": lambda x, y: jnp.bitwise_and(x, y),
            "input_values": [
        ...
        {
            "testcase": "jnp_bitwise_and_mixed_dtype",
            "callable": lambda x, y: jnp.bitwise_and(x, y),
            "input_values": [
                np.array([1, 2, 3], dtype=np.int32),
                np.array([True, False, True], dtype=np.bool_),
        ...
        {
            "testcase": "jnp_bitwise_and_broadcast",
            "callable": lambda x, y: jnp.bitwise_and(x, y),
        ...
        {
            "testcase": "bitwise_and_vmap_batching",
            "callable": lambda x, y: jax.vmap(jnp.bitwise_and)(x, y),
            "input_shapes": [(3, 4), (3, 4)],
            "input_dtypes": [np.int32, np.int32],
        },
    ],
)
class JnpBitwiseAndPlugin(PrimitiveLeafPlugin):
    _PRIM: ClassVar = _BITWISE_AND_PRIM
    _FUNC_NAME: ClassVar[str] = "bitwise_and"
    ...
    def lower(self, ctx: LoweringContextProtocol, eqn: core.JaxprEqn) -> None:
        ...
        x_val = ctx.get_value_for_var(
            x_var, name_hint=ctx.fresh_name("jnp_bitwise_and_x")
        )
        ...
        y_val = ctx.get_value_for_var(
            y_var,
            name_hint=ctx.fresh_name("jnp_bitwise_and_y"),
            prefer_np_dtype=prefer_dt,
        )
        out_spec = ctx.get_value_for_var(
            out_var, name_hint=ctx.fresh_name("jnp_bitwise_and_out")
        )
        ...
        x_ready = _cast_to_dtype(
            ...
            x_val,
            np_dtype=out_dtype,
            name_hint="jnp_bitwise_and_x_cast",
        )
        y_ready = _cast_to_dtype(
            ctx,
            y_val,
            np_dtype=out_dtype,
            name_hint="jnp_bitwise_and_y_cast",
        )
        ...
        if callable(producer) and producer() is not None:
            desired_name = ctx.fresh_name("jnp_bitwise_and_out")

        if np.issubdtype(out_dtype, np.bool_):
            result = ctx.builder.And(x_ready, y_ready, _outputs=[desired_name])
        else:
            result = ctx.builder.BitwiseAnd(x_ready, y_ready, _outputs=[desired_name])
...
def _bitwise_and_impl(*args: object, **kwargs: object) -> object:
    orig = get_orig_impl(JnpBitwiseAndPlugin._PRIM, JnpBitwiseAndPlugin._FUNC_NAME)
    return orig(*args, **kwargs)


def _bitwise_and_batch_rule(
    args: tuple[Any, ...], dims: tuple[Any, ...], **params: Any
) -> tuple[Any, Any]:
    return cast(
        tuple[Any, Any],
        broadcast_batcher_compat(JnpBitwiseAndPlugin._PRIM, args, dims, **params),
    )


batching.primitive_batchers[JnpBitwiseAndPlugin._PRIM] = _bitwise_and_batch_rule
