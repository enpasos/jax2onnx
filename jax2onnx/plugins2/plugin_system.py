...
_IN_FUNCTION_BUILD: ContextVar[set[str]] = ContextVar("_IN_FUNCTION_BUILD", default=set())
...
class FunctionPlugin(PrimitivePlugin):
    ...
    def patch_info(self) -> dict[str, Any]:
        if inspect.isclass(self.target):
            ...
            raise TypeError(f"Unsupported target type for patching: {type(self.target)}")
    ...
    def _aval_to_shaped_array(aval):
        ...
        raise TypeError(f"Cannot convert abstract value of type {type(aval)} to ShapedArray.")
    ...
    def _abstract_eval_with_kwargs(self, *args, **kwargs):
        ...
        specs = [
            jax.ShapeDtypeStruct(arg.shape, arg.dtype) if isinstance(arg, ShapedArray) else arg
            for arg in args
        ]
        ...
        def patch(original_call):
            ...
            def wrapped(*args, **kwargs):
                ...
                if expects_self:
                    ...
                    hits = set(_ONNX_FN_HITS.get()); hits.add(self.name.split("::", 1)[1]); _ONNX_FN_HITS.set(hits)
                    return primitive.bind(*args[1:], **{**kwargs, "instance_key": instance_key})
                else:
    ...
    def _lower_and_call(self, converter: Any, eqn: Any, params: dict[str, Any]):
        ...
        fkey = FunctionKey(qualified_name=qualname, input_sig=in_sigs_t, capture_sig=capture_sig)
        ...
        if fdef is None:
            ...
            default_float = np.float64 if getattr(ctx, "enable_double_precision", False) else np.float32
            ...
            for v in eqn.invars:
                aval = getattr(v, "aval", None)
                sds.append(jax.ShapeDtypeStruct(getattr(aval, "shape", ()), getattr(aval, "dtype", np.float32)))
            ...
            if len(jpr_f.invars) != len(in_vals_child):
                raise RuntimeError("[onnx_function] arity mismatch between traced invars and function inputs")
...
def register_example(**metadata: Any) -> dict[str, Any]:
    ...
    if _legacy_register_example is not None:
        try:
            ...
            logger.debug("Mirroring examples2 entry %r into legacy registry failed", metadata, exc_info=True)

    return metadata


def register_primitive(**metadata: Any) -> Callable[[type[PrimitiveLeafPlugin]], type[PrimitiveLeafPlugin]]:
...
def _import_tree(root_dir: Path, pkg_prefix: str) -> None:
    ...
    for py in root_dir.rglob("*.py"):
        ...
        try:
            ...
            logger.warning("Skipping import of %s due to error: %s", modname, e, exc_info=True)

    # 2) pkgutil walk (dup-safe)
    for _, module_name, _ in pkgutil.walk_packages([str(root_dir)], prefix=f"{pkg_prefix}."):
        try:
            importlib.import_module(module_name)
        except Exception as e:
            logger.warning("Skipping pkgutil import of %s due to error: %s", module_name, e, exc_info=True)
