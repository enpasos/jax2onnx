"""Shared helpers for reduction primitives in plugins2."""
...
def _normalize_axes(axes: Optional[Iterable[int]], rank: int) -> Optional[tuple[int, ...]]:
    if axes is None:
        return None
    normalized: list[int] = []
    for ax in axes:
...
def _maybe_cast_input(
    ...
    cast_val = ir.Value(
        ...
        type=ir.TensorType(dtype_enum, tensor.shape),
        shape=tensor.shape,
    )
    ctx.add_node(
        ir.Node(
...
def lower_reduction(
    ...
    *,
    ...
    if aval_dtype is not None:
        out_dtype_enum = _dtype_to_ir(np.dtype(aval_dtype), ctx.builder.enable_double_precision)
