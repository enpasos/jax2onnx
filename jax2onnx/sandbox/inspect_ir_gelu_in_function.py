...
def dump_gelu_attrs_in_ir(ir_model):
    # Top graph
    print("TOP GRAPH:")
    nodes = getattr(ir_model.graph, "nodes", None) or getattr(ir_model.graph, "_nodes", [])
    for n in nodes:
        if getattr(n, "op_type", "") == "Gelu":
            attrs = getattr(n, "attributes", None) or getattr(n, "_attributes", None)
            print("  Gelu attrs (top):", {getattr(a, "name", getattr(a, "key", None)): getattr(a, "value", None) for a in (attrs or [])})
    ...
    for fn in fvalues:
        print(f"FUNCTION: domain='{getattr(fn, 'domain', '')}', name='{getattr(fn, 'name', '')}'")
        fn_nodes = getattr(fn.graph, "nodes", None) or getattr(fn.graph, "_nodes", [])
        for n in fn_nodes:
            if getattr(n, "op_type", "") == "Gelu":
                attrs = getattr(n, "attributes", None) or getattr(n, "_attributes", None)
                print("  Gelu attrs (fn):", {getattr(a, "name", getattr(a, "key", None)): getattr(a, "value", None) for a in (attrs or [])})
