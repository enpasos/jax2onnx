...
import jax, jax.numpy as jnp, onnxruntime as ort
...
assert np.allclose(forward(x_init),
                   module.apply(variables, x_init),
                   rtol=1e-6, atol=1e-6)
...
onnx_model = jax2onnx.to_onnx(forward,
                              [x_init],       # concrete array
                              model_name="DenseReluFrozen")
...
np.testing.assert_allclose(np.array(forward(x_init)), onnx_out,
                           rtol=1e-5, atol=1e-5)
