...
def get_w_b(model):
    ...
    A_name, B_name = gemm.input[0], gemm.input[1]
    C_name = gemm.input[2] if len(gemm.input) >= 3 else None
    transB = 0
    for a in gemm.attribute:
        if a.name == "transB": transB = oh.get_attribute_value(a)
    W = numpy_helper.to_array(inits[B_name])
    if transB: W = W.T  # ensure shape (K,M)
...
def choose_input_shape(model, K):
    ...
    dd = [d.dim_value if d.HasField("dim_value") and d.dim_value>0 else 3 for d in dims]
    if len(dd) >= 2: dd[-1] = K
    elif len(dd) == 1: dd[0] = K
    return tuple(dd)
...
def stats(a, b):
    ...
    ad = np.abs(d); idx = np.unravel_index(ad.argmax(), ad.shape)
    from statistics import median
    return dict(max=float(ad[idx]), mean=float(ad.mean()), median=float(median(ad.ravel())), max_idx=tuple(int(i) for i in idx))
