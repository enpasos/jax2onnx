...
def test_dino_rotary_process_heads_emits_rotary_lowering():
    ...
    ir_model = to_onnx(block_call, inputs, return_mode="ir", opset=21)
...
def test_dino_rotary_process_heads_emits_fused_rotary_embedding():
    dim = 64
    num_heads = 4
    seq_len = 16
    ...
    inputs = [jax.ShapeDtypeStruct((1, seq_len, dim), jnp.float32)]
    ir_model = to_onnx(block_call, inputs, return_mode="ir", opset=23)

    rotary_nodes = _count_nodes(ir_model, "RotaryEmbedding")
    assert (
        rotary_nodes == 0
    ), "process_heads fallback uses decomposed rotary even at opset 23"
