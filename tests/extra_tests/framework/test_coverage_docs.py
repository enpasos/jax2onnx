...
def test_examples_doc_lists_registered_example_testcases() -> None:
    from tests.t_generator import get_plugin_grouping

    examples_text = _read_doc("docs/user_guide/examples.md")
    missing: list[str] = []

    for (context, component), testcases in get_plugin_grouping(reset=True).items():
        if not context.startswith("examples"):
            continue
        for testcase in testcases:
            testcase_name = str(testcase["testcase"])
            if f"[`{testcase_name}`]" not in examples_text:
                missing.append(f"{context}.{component}.{testcase_name}")

    assert not missing, "Examples doc is missing registered testcases:\n" + "\n".join(
        sorted(missing)
    )
