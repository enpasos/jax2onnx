...
def _read_meta(eqx_path: Path) -> dict:
    ...
    if not meta_path.exists():
        raise FileNotFoundError(f"Metadata file missing for Equinox archive: {meta_path}")
...
def _run_eqx(model: Transformer, tokens: np.ndarray) -> np.ndarray:
...
def _topo_sort_custom_functions(model: onnx.ModelProto) -> None:
    ...
    key_for = lambda f: (f.domain, f.name)
...
def _run_onnx_reference(onnx_path: Path, tokens: np.ndarray) -> np.ndarray:
...
def _run_onnx_ort(onnx_path: Path, tokens: np.ndarray) -> np.ndarray:
    if ort is None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eqx", required=True, type=Path, help="Path to Equinox .eqx file (with .meta.json).")
    parser.add_argument("--onnx", required=True, type=Path, help="Path to ONNX file.")
    parser.add_argument("--config", required=True, type=Path, help="GPT-OSS config JSON (for vocab size).")
    parser.add_argument("--prompt", required=True, help="Prompt text.")
    parser.add_argument("--sequence-length", type=int, default=32, help="Sequence length to pad/truncate to.")
    parser.add_argument(
        "--runtime",
...
def main() -> int:
    ...
    eqx_logits = _run_eqx(eqx_model, padded)

    if args.runtime == "reference":
        onnx_logits = _run_onnx_reference(args.onnx.expanduser(), padded)
    else:
        onnx_logits = _run_onnx_ort(args.onnx.expanduser(), padded)
    ...
    print(f"Diff all tokens: max={max_diff:.4e}, mean={mean_diff:.4e}, median={median_diff:.4e}")
    print(f"Diff last token: max={last_max:.4e}, mean={last_mean:.4e}, median={last_med:.4e}")
    return 0
