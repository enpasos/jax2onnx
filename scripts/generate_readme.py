...
def _fail_on_missing_markers(unmarked: list[tuple[Path, str, str]]) -> None:
    """Abort when a target lost its autogenerated markers.

    Without them ``replace_markers`` hands the file back unchanged, so the run
    would report success while writing nothing — and the dropped-row guard
    cannot inspect a section that it never found.
    """
    lines = [
        "❌ Cannot regenerate the coverage tables: the autogenerated markers are",
        "   missing, so there is no section to replace and the dropped-row guard",
        "   cannot run.",
        "",
    ]
    for path, start, end in sorted(unmarked, key=lambda item: str(item[0])):
        lines.append(f"   {path.name} is missing {start} / {end}")
    lines += [
        "",
        "   Restore the marker pair around the generated table, then re-run.",
    ]
    raise SystemExit("\n".join(lines))
...
def update_coverage_tables(
    ...
    unmarked: list[tuple[Path, str, str]] = []
    for path, new_section, start, end in targets:
        if not path.exists():
            logging.warning(f"⚠️ {path.name} not found.")
            continue
        content = path.read_text(encoding="utf-8")
        if _extract_section(content, start, end) is None:
            unmarked.append((path, start, end))
            continue
    ...
    if unmarked:
        _fail_on_missing_markers(unmarked)
