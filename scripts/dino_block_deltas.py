#!/usr/bin/env python3
# scripts/dino_block_deltas.py

"""
Compute per-block activation deltas between the simplified examples.eqx_dino
VisionTransformer (using mapped weights) and the converted Equimo Equinox
VisionTransformer.

This uses:
  - examples.eqx_dino.VisionTransformer.block_debug_outputs for the example
  - A manual per-block trace through equimo.models.vit.VisionTransformer

The output is a compact table listing max absolute deltas per stage for each
block, suitable for pasting into work notes.
...
def _example_block_debug(model, x_bchw: np.ndarray) -> List[Dict[str, jax.Array]]:
    ...
    # debugs is List[Dict[str, Array]] with shape (B, T, D); squeeze batch
...
def _equimo_block_debug(
    ...
    # Patch embed expects CHW input; returns (C', H', W')
    pe = model.patch_embed(jnp.asarray(x_chw))
    # Flatten into (T, D)
    patch_tokens = jnp.transpose(pe.reshape(pe.shape[0], -1), (1, 0))
    cls = model.cls_token  # (1, D)
    regs = model.reg_tokens  # (R, D)
    prefix = jnp.concatenate([cls, regs], axis=0)
    tokens = jnp.concatenate([prefix, patch_tokens], axis=0)  # (T_total, D)

    # Prepare rotary caches
    # Derive heads from first inner block
...
def _max_abs_delta(a: jax.Array, b: jax.Array) -> float:
    return float(jnp.max(jnp.abs(a - b)))


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--image", type=Path, required=True, help="Input image file")
    p.add_argument(
        "--eqx",
        type=Path,
        default=Path(
        ...
        help="Mapped example .eqx weights (defaults to Equimo cache).",
    )
    p.add_argument(
        "--variant",
        type=str,
        default="dinov3_vits16_pretrain_lvd1689m",
        help="Equimo/Meta variant identifier",
    )
    ...
    # Example debug (BCHW => squeeze batch in helper)
    ...
    print(
        f"Variant: {args.variant} | Tokens={meta['tokens']} Dim={meta['dim']} Heads={meta['heads']} Prefix={meta['prefix']}"
    )
    print("| Block | " + " | ".join(stages) + " |")
    print("|------:|" + "|".join(["--------------" for _ in stages]) + "|")
    for i, (a, b) in enumerate(zip(ex_debug, eq_debug)):
        ...
        for s in stages:
            ...
            da = a[s]
            db = b[s]
            # Align shapes: example is (T, D), equimo is (T, D)
            if da.shape != db.shape:
                # Fallback to min common tokens if any mismatch
                t = min(da.shape[0], db.shape[0])
                d = min(da.shape[1], db.shape[1])
                da = da[:t, :d]
                db = db[:t, :d]
            deltas.append(_max_abs_delta(da, db))
        # Format row
        cells = [f"{d:.2e}" if np.isfinite(d) else "-" for d in deltas]
